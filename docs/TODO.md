...
- [ ] `.env.local`에 실제 `YOUTUBE_API_KEY` 설정 후 `npm run dev` *(사용자 수동 — 키 필요)*
- [ ] **수동 E2E 시나리오**(자동화 대신): 검색 → 결과 카드 표시 → mp3 변환·다운로드·재생 확인 → mp4 변환·다운로드·재생 확인 *(사용자 수동)*
- [ ] 실패 경로 수동 확인: 빈결과 메시지, 검색 에러 배너+재시도, 변환 실패 인라인+재시도, 변환 중 다른 카드 사용 가능, 탭 닫기 시 중단(프로세스/temp 정리) *(사용자 수동)*
- [ ] 접근성/엣지: aria 라벨·포커스(토큰)·키보드 조작, 긴 제목 truncate, 썸네일 실패 placeholder *(코드/테스트 구현됨 — 수동 클릭 확인만 남음)*
- [x] **`.gitkeep` 잔재 정리**: 실제 파일이 생긴 폴더의 placeholder 제거(Phase 진행으로 모든 스캐폴딩 폴더가 채워졌으면 `src/**/.gitkeep` 전부 삭제)
- [x] `API.md` 검증 체크리스트 5항목 재확인, 전체 게이트 그린(`typecheck`/`lint`/`test` + `build`)
- [x] (선택) `README.md`: 빠른 시작(키 설정 → `npm run dev`) + ToS 디스클레이머(개인·학습용, 공개 배포 금지)
